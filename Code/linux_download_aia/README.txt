...
linux_download_aia.py then queries the table event_df_main.csv and downloads one image for each
space weather event found in event_df_main.csv that matches the requirements specified by the 
7 input arguments given at runtime. It also downloads an additional image, one per located event, at a time where there was no space weather event present. These images are then uploaded to Google Drive and deleted from the local machine. 
...
(Note that the upper bounds on the year/month/day are exclusive. So the above script would not download any data from 2016 or the month of May)
